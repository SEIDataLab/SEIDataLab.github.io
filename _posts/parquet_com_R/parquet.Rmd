...
A Apache Foundation tem mantido o projeto Arrow, que de acordo com o site do projeteo (https://arrow.apache.org/): *"A cross-language development platform for in-memory analytics"*. De forma geral, poderiamos dizer que o Arrow é uma framework para desenvolver aplicações que processam e transportam grandes conjuntos de dados. O Apache Arrow ajuda a melhorar desempenho de algoritmos analíticos e dá eficiência na movimentação de dados de uma sistema (ou linguagem de programação) para outra.

Felizmente, existe uma biblioteca do Apache Arrow no R, de nome `arrow`, que dentre várias funcionalidades, fornece funções de fácil utilização, para leitura e escrita de arquivos no formato parquet, respectivamente `read_parquet` e `write_parquet`. Para ter acesso a essas funções, primeiro instale a biblioteca `arrow`:
