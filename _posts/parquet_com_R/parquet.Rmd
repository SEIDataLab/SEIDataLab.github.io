...
Além da `arrow`, outras biblioteca também disponibilizam funções para leituras de arquivos `csv`. O próprio R Base tem a função `read.delim()` para ler arquivos no formato de valores separados por caracter, conforme script abaixo:

``` {.r}
matricula_NE <- read.delim(
  file = "./microdados_educacao_basica_2019/DADOS/DOCENTES_CO.CSV",
  delim = "|"
)
```
...
dataset            |   arrow  |   readr  |   Base   | data.table  
------------------ | -------: | -------: | -------: |   -------: 
DOCENTES_CO        |     5,37 |   16,64  |    37,46 |      3,91 
DOCENTES_NORDESTE  |    16,75 |   52,78  |   132,88 |      6,25       
DOCENTES_NORTE     |     6,56 |   21,23  |          |            
DOCENTES_SUDESTE   |    46,21 |   80,37  |          |            
DOCENTES_SUL       |    11,08 |   35,99  |          |            
ESCOLAS            |     2,50 |    9,27  |          |            
GESTOR             |     1,61 |    3,71  |          |            
TURMAS             |     7,42 |   27,41  |          |            
MATRICULA_CO       |    19,04 |   59,06  |          |            
MATRICULA_NORDESTE | 1.810,51 |     -    |          |            
MATRICULA_NORTE    |    31.73 |   74,97  |          |            
MATRICULA_SUDESTE  | 3.103,85 |     -    |          |            
MATRICULA_SUL      |    56,18 |     -    |          |            

Vale ressaltar que o `readr` não conseguiu ler o arquivo `MATRICULA_NORDESTE`, `MATRICULA_SUDESTE` e `MATRICULA_SUL`, justamente os mais massivos, tanto em termos de número de registros, quanto em termos de tamanho em disco.
